# General python coding practices — Equivalent Wave Height (EWH) from Wave Spectra

## About the project

See the pdf `EquivalentWaveHeigh.pdf`. Goal: follow the evolution in time of swell for a specific
frequency range by computing the "Equivalent Wave Height" (EWH) from 2D wave spectra:

```
EWH_f1-f2 = 4 * sqrt( integral_0^2pi integral_f1^f2 SPEC(f, theta) df d(theta) )
```

where `SPEC(f, theta)` is the 2-dimensional wave spectrum, `f` is the cyclic frequency (Hz), and
`theta` is the wave direction (integrated over all directions, 0 to 2*pi). Note: the source PDF
labels the frequency variable `omega` ("angular frequency"), but the actual NetCDF `freq` variable
is in `1/s` (Hz, cyclic frequency) and matches f1/f2 directly with no rad/s conversion needed — use
`f`/Hz throughout the code and docs, not `omega`/rad-s, to avoid a unit-conversion bug.

- Example frequency band: f1 = 0.04595011 Hz, f2 = 0.05559963 Hz (periods ~17.99-21.76 s). These
  values land exactly on two adjacent discrete bins of the model's `freq` array (index 3 and 4 of
  30, which is logarithmically spaced). The frequency integral is done via **trapezoidal
  integration between these two bin centers** (decided with the user 2026-08-18).
- Data source: MET Norway THREDDS wave spectra dataset `mywavewam3km_spectra`, e.g.
  `https://thredds.met.no/thredds/catalog/windsurfer/mywavewam3km_spectra/1995/02/catalog.html`,
  accessible via OPeNDAP (`.../dodsC/...`). One file per day (e.g. `SPC1995020100.nc`) already
  contains all 24 hourly time steps for that day — only one file needs to be fetched per day, not
  one per hour.
- Grid: `SPEC` has shape `[time=24, y=1, x=28713, freq=30, direction=24]`. `direction` has 24
  uniform 15-degree bins covering the full circle (must be converted to radians for the theta
  integral: `d(theta) = 2*pi/24` per bin). The spatial grid is a **flattened, unstructured** native
  WAM grid (`longitude`/`latitude` are 2D arrays over `(y, x)`, not a regular lat/lon grid) —
  contour plots must use `tricontourf` (triangulation), not `contourf`.
- Deliverables (first iteration, decided with the user 2026-08-18): contour maps of the spatial
  distribution of EWH_f1-f2 for the day 1995-02-01, one map per hourly time step (or as agreed when
  implementing). A time-series-at-a-point/region deliverable is a possible future iteration, not
  part of the first implementation.

## Goal of this folder and code organization

Work is split into consecutive, self-contained numbered task scripts (per the general convention
above):

- `00_fetch_spectra.py`: access/download the wave spectra `.nc` files from the THREDDS server for
  the dates/times of interest.
- `01_compute_ewh.py`: compute EWH_f1-f2 from the fetched spectra for the chosen frequency range.
- `02_plot_contours.py`: plot contour maps of EWH_f1-f2 for a given day.
...
- use established packages:
  ...
  - allowed third-party packages (conda-forge, pinned in `environment.yml`): `xarray`, `netCDF4`,
    `numpy`, `matplotlib`, `requests` (data access/handling); `loguru` (logging); `tqdm` (progress
    bars); `pytest` (tests); `mypy`, `ruff`, `pylint`, `flake8`, `codespell`, `complexipy` (checkers)
...
- use, or create if necessary, a dedicated environment for working on this, with name: "kivm_swell"; use only this env for running code here
